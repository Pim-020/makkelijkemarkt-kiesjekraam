...
export default class MarketListItem extends Component<{ market: Market; marketId: string }> {
  ...
  render() {
    ...
    return (
      <div key={marketId} className={`${this.state.market.phase || 'onbekend'} market-card`}>
        <Link
          to={{
        ...
        {this.state.market.phase && (
          <p>
            <span style={{ color: 'rgba(0, 0, 0, 0.45)' }}>fase:</span> {this.state.market.phase}
          </p>
        )}
        {this.state.market.stands && (
          <p>
            <span style={{ color: 'rgba(0, 0, 0, 0.45)' }}>kramen: {this.state.market.stands}</span>
          </p>
        )}
