import { IMarkt, IMarktindeling, IMarktondernemer, IMarktindelingSeed } from './markt.model';

import Indeling from './allocation/indeling';
import Ondernemer from './allocation/ondernemer';

const getListGroup = (indeling: IMarktindeling, ondernemer: IMarktondernemer) =>
    Ondernemer.isVast(ondernemer) ? 1 : indeling.aLijst.includes(ondernemer) ? 1 : 2;
...
export const calcToewijzingen = (markt: IMarkt & IMarktindelingSeed): IMarktindeling => {
    ...
    if (indeling.aLijst.length) {
        const aListQueue = indeling.ondernemers.filter(ondernemer => getListGroup(indeling, ondernemer) === 1);
    ...
    const bListQueue = indeling.ondernemers.filter(
        ondernemer => !indeling.aLijst.length || getListGroup(indeling, ondernemer) === 2,
    );
