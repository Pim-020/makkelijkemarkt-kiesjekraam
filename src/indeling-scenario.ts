...
const createRSVP = (ondernemer: IMarktondernemer, marktId: string, marktDate: string): IRSVP => ({
    marktId,
    marktDate,
...
const marktScenario = (seed: IMarktScenarioStub): IMarktScenario => {
    ...
    const marktId = '1';
    const marktDate = 'marktDate' in seed ? seed.marktDate : new Date().toISOString().slice(0, 10);
    ...
    markt.marktplaatsen = (seed.marktplaatsen || []).map(
        (data: IMarktplaatsStub | string): IMarktplaats => {
    ...
    markt.ondernemers = (seed.ondernemers || []).map(
        (data: IMarktondernemerStub = {}): IMarktondernemer => {
            const {
    ...
    if (seed.aanwezigheid) {
        markt.aanwezigheid = seed.aanwezigheid.map(
            (data: IRSVPStub): IRSVP => {
        ...
        const deFacto = deFactoAanmeldingen(markt.ondernemers, marktId, marktDate).filter(
            deFactoAanmelding => !markt.aanwezigheid.find(aanmelding => isEqualRSVPKey(aanmelding, deFactoAanmelding)),
        );

        markt.aanwezigheid = [...markt.aanwezigheid, ...deFacto];
    } else {
    ...
    markt.voorkeuren = (seed.voorkeuren || []).map(
        (data: IPlaatsvoorkeurStub): IPlaatsvoorkeur => {
            const ondernemer = findOndernemer(markt, data);
            if (!ondernemer) {
                throw Error('Ondernemer not found');
            }
